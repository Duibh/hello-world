// Import required classes and methods from the Scalafx library
...
object SpellPointTracker extends JFXApp {
  ...
  // Maps the caster level to the total number of spell points available at that level
  ...
  // Define an empty map that will be used to keep track of the number of spell points spent at each spell level
  var spellPoints: Map[Int, Int] = Map.empty

   // Define a function that sets the current caster level and initializes the spellPoints map with 0 spell points for each spell level. It also sets the maximum value for each spinner.
  def setCasterLevel(level: Int, spellCostPerLevel: Map[Int, Int], spellPointTotal: Map[Int, Int]): Unit = {
    ...
    spellLevelSpinners.foreach(spinner => {
      ...
      // Create a new SpinnerValueFactory that limits the spinner's value to a range of 0 to the maximum number of spell points available at the current caster level for the given spell level.
      val valueFactory = new SpinnerValueFactory.IntegerSpinnerValueFactory(0, spellPointTotal(level), 0)
      valueFactory.setWrapAround(true)

      // Add a listener to the spinner's valueProperty that updates the spellPoints map and the totalPointsLabel whenever the spinner's value changes. It also updates the maximum
      // value of all the other spinners based on the number of spell points remaining at the current caster level.
